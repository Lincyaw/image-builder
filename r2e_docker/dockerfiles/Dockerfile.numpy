FROM aibrix-docker-mirror-cn-beijing.cr.volces.com/library/ubuntu:22.04
...
RUN set -eux; \
    for i in 1 2 3; do \
        rm -rf testbed; \
        ( \
            git clone --no-single-branch https://github.com/numpy/numpy.git testbed; \
            cd testbed; \
            git checkout -f "${BASE_COMMIT}"; \
            test -f setup.py -o -f pyproject.toml \
        ) && break; \
        echo "git fetch base commit failed (attempt $i), retrying..."; \
        sleep $((i * 10)); \
    done; \
    test -d testbed/.git; \
    test -f testbed/setup.py -o -f testbed/pyproject.toml
